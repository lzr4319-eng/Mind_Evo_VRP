    ...
    def evaluate(self, plan: str) -> (float, List[str]): # type: ignore
        ...
        for i, route in enumerate(routes):
            ...
            try:
                ...
                feedback.append(f"错误：路线 {i+1} 包含无效的数据类型，无法处理。路线内容: {route}")
                return float('inf'), feedback
            # --- 防御性检查结束 ---
...
class MindEvolution:
    ...
    def _extract_json_str(text: str) -> str:
        """
        尝试从任意文本中提取首个合法 JSON 对象字符串。
        先直读，失败则用括号配对扫描，尽量还原纯 JSON（应对API偶尔夹带额外说明的情况）。
        """
    ...
    # ======= 严格符合指定 API 请求格式的 LLM 调用 =======
    # 仅改动调用方式；其余逻辑/提示词/遗传算法均不变
    def _prompt_llm(self, messages: List[Dict[str, str]], json_mode: bool = True) -> str:
        """
        将外部传入的多条 messages（可能包含 system/user）按顺序拼接成一个字符串，
        作为唯一的一条 user 消息的 content，严格匹配你要求的请求格式。
        """
        prompt_text = "\n\n".join(
        ...
        headers = {
            "Authorization": "Bearer sk-bitzklhaztgalwjrhudnngncdrazxxkejrzbpnlqqilyzzbk",
        ...
        try:
            ...
            return self._extract_json_str(content) if json_mode else content
    ...
    def initialize_population(self):
        ...
        messages = [
            {'role': 'system', 'content': '你是一个善于生成多样化、有效解决方案的物流规划AI，严格遵循JSON输出格式和自检要求。'},
    ...
    def _selection(self, ensure_include: Optional[Individual] = None) -> List[Individual]:
        """
        选择父代（锦标赛选择），并确保 ensure_include(若给定) 一定被纳入父代集中。
        这样可以保证「至少有一次重组的父代集合包含全局最优解」。
        """
        ...
        if not source_population:
            # 即使种群为空，也要保证返回 ensure_include（如果传入）
            return [ensure_include] if ensure_include is not None else []
        ...
        # --- 强制包含 ensure_include 的逻辑 ---
        if ensure_include is not None:
            # 如果 parents 为空，直接加入 ensure_include
            if len(parents) == 0:
                parents.append(ensure_include)
            # 如果 ensure_include 不在 parents 中
            elif all((pi.plan != ensure_include.plan or pi.fitness != ensure_include.fitness) for pi in parents):
                if len(parents) < N_PARENTS_FOR_RECOMBINATION:
                    parents.append(ensure_include)
                else:
                    # 替换掉父集中最差的一个，使 ensure_include 进入父代
                    worst_idx = max(range(len(parents)), key=lambda i: parents[i].fitness)
                    parents[worst_idx] = ensure_include
        # ------------------------------------
    ...
    def _recombine_and_refine(self, parents: List[Individual]) -> Individual:
        ...
        for i, p in enumerate(parents):
            parent_info_lines.append(f"父代方案 {i+1} (成本: {p.fitness:.2f}): {p.plan}")
        parent_info = "\n".join(parent_info_lines)
        ...
        try:
            ...
            print(f"  [精炼失败]: {e}")
            # 回退：返回父代中最优的一个，防止无效个体扩散
            return min(parents, key=lambda ind: ind.fitness) if parents else Individual('{"routes": []}')
    ...
    def run(self):
        ...
        for gen in range(N_GENERATIONS):
            ...
            # 精英主义：直接保留全局最优个体
            new_population = [self.best_solution_so_far]

            # 标记：本代是否已使用“包含全局最优”的父代集合进行过一次重组
            used_best_parent_once = False

            while len(new_population) < POPULATION_SIZE:
                # 第一次生成子代时，强制把全局最优纳入父代集合
                ensure = self.best_solution_so_far if not used_best_parent_once else None
                parents = self._selection(ensure_include=ensure)

                if not parents:
                    # 如果没有可选父代（极端情况），对全局最优做一次变异探索
                    print("\n种群多样性不足，对最优解进行变异...")
                    ...
                    if ensure is not None:
                        # 若本次父代包含了全局最优，标记为已使用
                        for pi in parents:
                            if pi.plan == self.best_solution_so_far.plan and pi.fitness == self.best_solution_so_far.fitness:
                                used_best_parent_once = True
                                break
